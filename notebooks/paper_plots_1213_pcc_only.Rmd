...
figure_path = 'reports/figures/0404_paperfigure_sensors/'
```
...
plt.savefig(f'{figure_path}//tmp_nwsens.png',bbox_inches='tight')
...
ggsave(glue('{figure_path}//tmp_eff_deg_nolegend.png'), plot = eff_deg_comp+theme(legend.position='none'),width=4,height=4)
ggsave(glue('{figure_path}//tmp_eff_deg_withlegend.png'), plot = eff_deg_comp,width=6,height=4)
...
ggsave(glue('{figure_path}//tmp_eff_eff.png'), plot = eff_eff_comp+theme(legend.position='none'),width=4,height=4)
...
sens_deg_comp <- ggplot(sensors_pcc %>% drop_na() %>% right_join(py$e_pcc$df)%>% mutate(go_group = factor(replace_na(as.character(go_group),"Non-sensor genes")),go_group=fct_reorder(go_group, sens),go_group=fct_relevel(go_group,'Non-sensor genes',after=Inf)), aes(x = go_group, y = deg, fill = go_group)) +
  ...
  ggsave(glue('{figure_path}//sens_deg_withlegend.png'), plot=sens_deg_comp+theme(legend.position='bottom'),width=6,height=4)
ggsave(glue('{figure_path}//sens_deg.png'), plot=sens_deg_comp+theme(legend.position='none'),width=4,height=4)
...
sens_sens_comp <- ggplot(sensors_pcc %>% drop_na() %>% mutate(go_group = fct_reorder(go_group, sens)) %>%
  ...
  theme(
        ...
        axis.text.y=element_text(size=10),
        axis.text.x = element_blank(), legend.title = element_blank())
ggsave(glue('{figure_path}//sens_sens.png'), plot=sens_sens_comp+theme(legend.position='none'),width=4,height=4)
