...
# %%
with open(f'../data/interim/{nw_type}/{nw_type}_enm_object_corr.pickle','rb') as f:
    e_pcc = pickle.load(f)
...
# %%
#e_pcc.spring_pos(seed=1001)
pos = nx.spring_layout(e_pcc.graph_gc,k=0.4,seed=12)
nx.set_node_attributes(e_pcc.graph_gc, pos, 'pos')
...
# %%
sensors_pcc = pd.read_csv(f'../data/interim/{nw_type}/{nw_type}_sensors_df_corr.csv')
...
e_pcc.plot_network_spring(ax=ax,
                          node_size=1,
                          node_color='black', edge_alpha=0.6,
...
# %%
effector_pcc = pd.read_csv(f'../data/interim/{nw_type}/{nw_type}_effectors_df_corr.csv')
