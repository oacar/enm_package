...
with open(f'data/interim/pcc_0909/pcc.pickle','rb') as f:
    e_pcc = pickle.load(f)
with open(f'data/interim/psn_1201/psn.pickle','rb') as f:
    e_psn = pickle.load(f)
with open(f'data/interim/npdp_1211/npdp.pickle','rb') as f:
    e_nsdp = pickle.load(f)
#with open(f'data/interim/nsdp_1205/nsdp.pickle','rb') as f:
#    e_nsdp = pickle.load(f)
#with open(f'data/interim/psn_1205/psn.pickle','rb') as f:
#    e_psn = pickle.load(f)
...
nsdp_df_random  = data.table::rbindlist(lapply(py$e_nsdp$e_list, function(x) x$df),fill=T,idcol='random_id')%>%mutate(random_id = as.factor(random_id))
psn_df_random  = data.table::rbindlist(lapply(py$e_psn$e_list, function(x) x$df),fill=T,idcol='random_id')%>%mutate(random_id = as.factor(random_id))
...
nsdp_df = py$nsdp_df%>%mutate(data='nsdp')%>%mutate(sens_norm = sens/nrow(.),eff_norm = eff/nrow(.), group = 'real')%>%bind_rows(
    nsdp_df_random%>%mutate(data='nsdp')%>%mutate(sens_norm = sens/nrow(py$nsdp_df),eff_norm = eff/nrow(py$nsdp_df), group = 'random')
)%>%mutate(group=fct_relevel(group, c('real','random')))
psn_df = py$psn_df%>%mutate(data='psn')%>%mutate(sens_norm = sens/nrow(.),eff_norm = eff/nrow(.), group = 'real')%>%bind_rows(
    psn_df_random%>%mutate(data='psn')%>%mutate(sens_norm = sens/nrow(py$psn_df),eff_norm = eff/nrow(py$psn_df), group = 'random')
)%>%mutate(group=fct_relevel(group, c('real','random')))
combined_df = bind_rows(pcc_df, nsdp_df,psn_df)%>%
    mutate(data= fct_relevel(data, c('pcc','nsdp','psn')))
```
...
#annotate("text", x = 0.000001, y = 0.2, label=sprintf("p = %#.2f", eff_p), fontface='italic', size = 6)
nsdp_degree_dens <- 
    plot_density_prs(nsdp_df, deg, xlab='Degree')
nsdp_eff_dens <- 
    plot_density_prs(nsdp_df, eff_norm, xlab='Effectiveness')
nsdp_sensitivity_dens <- 
    plot_density_prs(nsdp_df, sens_norm, xlab='Sensitivity')
title <- cowplot::ggdraw() + cowplot::draw_label("Yeast coexpression network", fontface='bold')
nsdp_dens_comb = cowplot::plot_grid(nsdp_degree_dens+theme(legend.position='none'), 
                                nsdp_eff_dens+theme(legend.position='none'), 
                                nsdp_sensitivity_dens+theme(legend.position='none'), labels=c('G','H','I'),ncol=3)
nsdp_dens_comb_title = cowplot::plot_grid(title, nsdp_dens_comb, nrow=2 ,rel_heights=c(0.1,0.9))
psn_degree_dens <- 
    plot_density_prs(psn_df, deg, xlab='Degree')
psn_eff_dens <- 
    plot_density_prs(psn_df, eff_norm, xlab='Effectiveness')
psn_sensitivity_dens <- 
    plot_density_prs(psn_df, sens_norm, xlab='Sensitivity')
title <- cowplot::ggdraw() + cowplot::draw_label("HuRI Profile similarity", fontface='bold')
psn_dens_comb = cowplot::plot_grid(psn_degree_dens+theme(legend.position='none'), 
                                psn_eff_dens+theme(legend.position='none'), 
                                psn_sensitivity_dens+theme(legend.position='none'), labels=c('J','K','L'),ncol=3)
psn_dens_comb_title = cowplot::plot_grid(title, psn_dens_comb, nrow=2 ,rel_heights=c(0.1,0.9))
dens_lgn = cowplot::get_legend(pcc_sensitivity_dens)
dens_plots = cowplot::plot_grid(pcc_dens_comb_title,
                                nsdp_dens_comb_title,
                                psn_dens_comb_title,nrow=3)
cowplot::plot_grid(dens_plots, dens_lgn,nrow=2,rel_heights=c(.9,.1))
ggsave('reports/figures/paper_figures_1211_withcoex/fig1.png', width=8,height=12)
...
#    facet_grid(rows = vars(group), scales='free', labeller =labeller(group= real_random_labels ) ) +
    ...
    theme(strip.text.y = element_text(angle = 0))+
    scale_y_continuous(labels = scales::label_scientific(digits=1))
nsdp_deg_eff = nsdp_df %>% filter(group=='real')%>%#pivot_longer(cols=sens_norm:eff_norm, names_to="prs_data_type", values_to="prs_data_value")%>%
    ggplot(aes(x=deg, y=eff_norm))+
    geom_point()+
#    facet_grid(rows = vars(group), scales='free', labeller =labeller(group= real_random_labels ) ) +
    theme_bw()+
    ...
    theme(strip.text.y = element_text(angle = 0))+
    scale_y_continuous(labels = scales::label_scientific(digits=1))
nsdp_deg_sens = nsdp_df %>% filter(group=='real')%>%#pivot_longer(cols=sens_norm:eff_norm, names_to="prs_data_type", values_to="prs_data_value")%>%
    ggplot(aes(x=deg, y=sens_norm))+
    geom_point()+
#    facet_grid(rows = vars(group), scales='free', labeller =labeller(group= real_random_labels ) ) +
    theme_bw()+
    ...
    theme(strip.text.y = element_text(angle = 0))+
    scale_y_continuous(labels = scales::label_scientific(digits=1))
psn_deg_eff = psn_df %>% filter(group=='real')%>%#pivot_longer(cols=sens_norm:eff_norm, names_to="prs_data_type", values_to="prs_data_value")%>%
    ggplot(aes(x=deg, y=eff_norm))+
    geom_point()+
#    facet_grid(rows = vars(group), scales='free', labeller =labeller(group= real_random_labels ) ) +
    theme_bw()+
    ...
    theme(strip.text.y = element_text(angle = 0))+
    scale_y_continuous(labels = scales::label_scientific(digits=1))
psn_deg_sens = psn_df %>%filter(group=='real')%>% #pivot_longer(cols=sens_norm:eff_norm, names_to="prs_data_type", values_to="prs_data_value")%>%
    ggplot(aes(x=deg, y=sens_norm))+
    geom_point()+
#    facet_grid(rows = vars(group), scales='free', labeller =labeller(group= real_random_labels ) )+
    theme_bw()+
...
deg_scatter_plots <- cowplot::plot_grid(pcc_deg_eff, pcc_deg_sens,
                   nsdp_deg_eff, nsdp_deg_sens,
                   psn_deg_eff, psn_deg_sens, labels = c('A','','B','','C','','D',''),
                   ncol=2)
ggsave('reports/figures/paper_figures_1211_withcoex/fig2.png',plot=deg_scatter_plots, width=10,height=10)

```

```{r}
pcc_rewire_df = read_csv('data/interim/pcc_0909/rewire_data.csv')
nsdp_rewire_df = read_csv('data/interim/npdp_1211/rewire_data.csv')
psn_rewire_df = read_csv('data/interim/psn_1201/rewire_data.csv')

rewire_df = pcc_rewire_df%>%mutate(data='pcc')%>%bind_rows(
                                                           nsdp_rewire_df%>%mutate(data='nsdp'))%>%
        bind_rows(psn_rewire_df%>%mutate(data='psn'))
```

```{r}
network_labels  <- c('Yeast GI Profile Similarity','Yeast Coex','HuRI')
...
names(facet_labels) <- c("pcc",'nsdp','psn')

combined_rewire_df <- rewire_df%>%select(data, eff_deg_pearson,sens_deg_spearman)%>%mutate(type = 'random')%>% 
    bind_rows(
    data.frame(sens_deg_spearman= c(cor(pcc_df[pcc_df$group=='real',]$deg, pcc_df[pcc_df$group=='real',]$sens,method='spearman'),cor(nsdp_df[nsdp_df$group=='real',]$deg,nsdp_df[nsdp_df$group=='real',]$sens,method='spearman'),cor(psn_df[psn_df$group=='real',]$deg,psn_df[psn_df$group=='real',]$sens,method='spearman')), 
               eff_deg_pearson= c(cor(pcc_df[pcc_df$group=='real',]$deg, pcc_df[pcc_df$group=='real',]$eff,method='pearson'),cor(nsdp_df[nsdp_df$group=='real',]$deg,nsdp_df[nsdp_df$group=='real',]$eff,method='pearson'),cor(psn_df[psn_df$group=='real',]$deg,psn_df[psn_df$group=='real',]$eff,method='pearson')), type='real',data=c('pcc','nsdp','psn')))%>%
    mutate(data= fct_relevel(data, c('pcc','nsdp','psn')))
...
ggsave('reports/figures/paper_figures_1211_withcoex/fig3.png',width=10,height=12)
```
...
nsdp_deletion = read_csv('data/interim/deletion_analysis_0911/nsdp_deletion_100.csv')%>%
    pivot_wider(id_cols=c('range','variable_0'),names_from='variable_1',values_from='value')%>%
    mutate(range = range/max(range), gc_size = gc_size/max(gc_size), group='nsdp')
psn_deletion = read_csv('data/interim/deletion_analysis_0911/psn_deletion_100.csv')%>%
    pivot_wider(id_cols=c('range','variable_0'),names_from='variable_1',values_from='value')%>%
    mutate(range = range/max(range), gc_size = gc_size/max(gc_size), group='psn')

deletion_df  <- bind_rows(pcc_deletion, nsdp_deletion, psn_deletion)%>%mutate(group = fct_relevel(group,c('pcc','nsdp','psn')))
...
auc_plot_nsdp <- auc_data%>%
    ggplot(aes(x=variable_0,y=nsdp))+
...
auc_plot_psn<- auc_data%>%
    ggplot(aes(x=variable_0,y=psn, alpha = (variable_0 == "eff_sorted_deletion" ) ))+
...
deletion_plot_nsdp <-  nsdp_deletion%>%   mutate(variable_0 = fct_relevel(variable_0, levels(auc_data$variable_0)))%>%
ggplot(aes(x=range, y= gc_size, color = variable_0 ,alpha = (variable_0 == "eff_sorted_deletion" )  ))+
#    geom_point(size=1)+
        geom_line(size=2)+
#    facet_grid(rows = vars(group), labeller = labeller(group=facet_labels))+
...
deletion_plot_psn <-  psn_deletion%>%   mutate(variable_0 = fct_relevel(variable_0, levels(auc_data$variable_0)))%>%
ggplot(aes(x=range, y= gc_size, color = variable_0 , alpha = (variable_0 == "eff_sorted_deletion"  ) ))+
#    geom_point(size=1)+
        geom_line(size=2)+
#    facet_grid(rows = vars(group), labeller = labeller(group=facet_labels))+
...
nsdp_with_inset = ggdraw(deletion_plot_nsdp+    theme(legend.position=''))+
  draw_plot(auc_plot_nsdp+theme(legend.position=''), .50, .62, .45, .35) +
  draw_plot_label(
    c("B", ""),
    c(0, 0.45),
    c(1, 0.95),
...
psn_with_inset = ggdraw(deletion_plot_psn+    theme(legend.position=''))+
  draw_plot(auc_plot_psn+theme(legend.position=''), .50, .62, .45, .35) +
  draw_plot_label(
    c("C", ""),
    c(0, 0.45),
    c(1, 0.95),
...
plot_grid(pcc_with_inset, nsdp_with_inset, psn_with_inset, ncol=3)%>%plot_grid(deletion_lgn,nrow=2, rel_heights=c(0.9,.1))
...
sensor_connectivity_df <- read_csv('data/interim/sensor_connectivity_1205/sensor_connectivity_df.csv')%>%
    mutate(data= fct_relevel(data, c('pcc','gi','nsdp','psn')))
#p3 = 
...
#combined_df%>%filter(group=='real'&data=='psn')%>%mutate(is_sensor = ifelse(sens_norm>quantile(sens_norm,0.99),TRUE,FALSE))%>%
