...
source("src/R/get.subclusters.dendrogram.R")
```
...
library(ggsignif)
library(glue)
figure_folder = 'reports/figures/CSHL_plots_022621/'
...
ggsave(glue('{figure_folder}/dens_comb.png', plot=pcc_dens_comb,width=8,height=8)
```
...
ggsave(glue("{figure_folder}/fig2.png"), plot = deg_scatter_plots, width = 10, height = 5)
```
...
p4 <-
  ggplot() +
      facet_grid(cols = vars(name), labeller = labeller(data=facet_labels))+
  #    geom_boxplot()+
  #    geom_jitter()+
  geom_boxplot(data = combined_rewire_df %>% filter(type != "real")%>%select(data, sens_deg_spearman,eff_deg_pearson)%>%pivot_longer(-data), 
...
#  geom_density(data = combined_rewire_df %>% filter(type != "real"), aes(x = sens_deg_spearman, y = ..scaled.., color = type), linetype = "dashed") +
  geom_point(data = combined_rewire_df %>% filter(type == "real")%>%select(data, sens_deg_spearman,eff_deg_pearson)%>%pivot_longer(-data), 
...
ggsave(glue("{figure_folder}/distr_plots.pdf"), plot= p4, width = 4, height = 10)
...
'#6a51a3',
'#4a1486'
)#col[1:4] # brewer.pal(n = 6, name = "Set1")
effector_colors <- c('#bae4b3',
'#74c476',
'#238b45')#col[5:8] # brewer.pal(n=3, name= "Set1_r")
```
...
sensors_pcc = e_pcc_old.sensors_df
...
```
# Dendrogram cluster network plots
```{python}
import copy
...
library(dendextend)
png(glue('{figure_folder}/row_dendrogram_colored.png'),width=400,height=400)
dend_row_plot <- dend_row[[2]] %>% 
    set("branches_k_color", k = 4,value = effector_colors[c(4,2,3,1)]) %>% 
    set("labels", NA) %>%    # change labelsk
    as.ggdend()%>%
    ggplot()+
    coord_flip()#    plot(xlab="", sub="")}
dend_row_plot
dev.off()
png(glue('{figure_folder}/col_dendrogram_colored.png'),width=400,height=400)
...
for itr, i in enumerate(sensor_order):
    ...
    legend_elements.append(
        Line2D([0], [0], marker='^', color='black', label=f'Column cluster ({itr+1})',
...
for itr, i in enumerate(effector_order):
    ...
    nx.draw_networkx_nodes(e_pcc.graph_gc, nodelist=orf_names_to_plot, node_size=200, pos=pos,
                          ...
                          linewidths=1)
    legend_elements.append(
            Line2D([0], [0], marker='o', color='black', label=f'Row cluster ({itr+1})',
...
#import src
#import importlib
#importlib.reload(src.visualize.visualize)
...
#    print(orf_names_to_plot)
    nx.draw_networkx_nodes(e_pcc.graph_gc, nodelist=orf_names_to_plot, node_size=200, pos=pos,
                          ...
                          linewidths=1)
    legend_elements.append(
        Line2D([0], [0], marker='^', color='black', label=f'Column Cluster ({i})',
                              markerfacecolor=sensor_colors[itr], markersize=12, linestyle="None")
    )
nx.draw_networkx_nodes(e_pcc.graph_gc, nodelist=effector_pcc.orf_name.values, node_size=100, pos=pos,
                      node_color='black',
