...
random_dfs_100<- random_dfs_100%>%mutate(eff_norm = eff/nrow(pcc_df), sens_norm=sens/nrow(pcc_df))
```
```{r}
...
#    facet_grid(rows=vars(data))+
    ...
    geom_line(data=     random_dfs_100%>%dplyr::mutate(group='random'),aes(x=deg,y=..scaled..,fill=filename),stat = 'density',linetype = "dashed")+#, binwidth = 0.1) + 
    theme_bw()+
    xlab('Degree')+
    ylab('Density')+
    coord_trans(x = "log10")+
...
#    geom_density()+
    ...
    geom_line(data= random_dfs_100%>%mutate(group='random'),
               aes(x=eff_norm ,y=..scaled..,fill=filename),
...
#    geom_density()+
    geom_line(data= combined_df%>%filter(group=='real'),
               ...
               stat = 'density')+#, binwidth = 0.1) + 
    geom_line(data= random_dfs_100%>%dplyr::mutate(group='random'),
               aes(x=sens_norm,y=..scaled..,fill=filename),
               stat = 'density',linetype = "dashed")+#, binwidth = 0.1) + 
    #geom_histogram(binwidth=0.0001)+
    theme_bw()+
    xlab('Sensitivity')+
