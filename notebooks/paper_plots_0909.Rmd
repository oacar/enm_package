...
pcc_df = py$pcc_df%>%mutate(data='pcc')%>%mutate(sens_norm = sens/nrow(.),eff_norm = eff/nrow(.))
yuri_df = py$yuri_df%>%mutate(data='yuri')%>%mutate(sens_norm = sens/nrow(.),eff_norm = eff/nrow(.))
huri_df = py$huri_df%>%mutate(data='huri')%>%mutate(sens_norm = sens/nrow(.),eff_norm = eff/nrow(.))
...
{
 pcc_gc_ig = convert_networkx_to_igraph(gc, df)

png('tmp.png')
plot(pcc_gc_ig, vertex.size=1, vertex.label=NA, layout = pos)
dev.off()
 
}
pcc_gc = py$pcc_gc
pcc_gc_pos = matrix(unlist(py$pcc_gc_pos),ncol=2,byrow=TRUE)

pcc_gc_ig = convert_networkx_to_igraph(pcc_gc, pcc_df)

get_subnetwork  <- function(gc, subset){
    sub_gc_nodes = igraph::ego(gc, nodes= subset)%>%unlist%>%unique

    sub_gc  <- igraph::induced_subgraph(gc, sub_gc_nodes)

    sub_gc
}

pcc_sensors = pcc_df %>% filter(sens>quantile(sens,0.99))
pcc_sensor_sub = get_subnetwork(pcc_gc_ig, pcc_sensors$orf_name)
#plot_network_spring(pcc_gc, pcc_df, pcc_gc_pos)

png('tmp.png', res=150, height=3, width=3,units='in')
plot(pcc_sensor_sub, vertex.size = 2 , vertex.color = 'white', vertex.label.color='black',vertex.label.size=8, vertex.label.cex = 0.4, layout=igraph::layout.fruchterman.reingold)
dev.off()
```
