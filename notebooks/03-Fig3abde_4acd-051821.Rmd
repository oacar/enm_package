...
snakemake <- new("smake",input=list("sensor_connectivity_df"="../data/interim_bak_3/sensor_connectivity_df.csv", "pcc_df"='../data/interim_bak_3/pcc_df.csv',"sensors_pcc"="../data/interim_bak_3/sensors_df.csv",'effector_pcc'='../data/interim_bak_3/effectors_df.csv',
"rwr_row"="../data/interim/rwr_ranked_goa_rows.csv","prs_row"="../data/interim_bak_3/prs_ranked_goa_rows.csv",
"rwr_column"="../data/interim/rwr_ranked_goa_columns.csv","prs_column"="../data/interim_bak_3/prs_ranked_goa_columns.csv"))
```
# Load necessary data
```{r}
...
sensor_neighbor_degree_plot <- pcc_df %>%
  ...
  geom_boxplot() +
  theme_bw() +
    theme(axis.text=element_text(size=text_size),axis.title = element_text(size=label_size))+
...
sensor_connectivity_plot <-
  ...
  theme(legend.position = "right",axis.text=element_text(size=text_size),axis.title = element_text(size=label_size) ) +
...
sens_sens_comp <- ggplot(sensors_pcc %>% mutate(label = fct_reorder(label, sens)) %>%
  ...
  geom_signif(
    ...
    annotation = c("*","***" , "*" ,  "***"), y_position = c(0.005,0.007,  0.0053, 0.0075),
                                           textsize = 3#, 0.008)
  ) +
  xlab("") +
...
eff_deg_comp <- ggplot(effector_pcc %>% right_join(pcc_df)%>% 
  ...
  theme(
       axis.text.y=element_text(size=text_size),
        axis.title.y=element_text(size=label_size),legend.title = element_blank(), legend.text = element_text(margin = margin(t = 10, b = 10))) # , legend.spacing.y = unit(2,'in'))+
eff_deg_comp
```
...
eff_eff_comp <- ggplot(effector_pcc %>% #mutate(go_group = fct_reorder(go_group, eff)) %>%
  ...
  theme(
        axis.text.y=element_text(size=text_size),
        axis.title.y=element_text(size=label_size), legend.title = element_blank())
eff_eff_comp
```
...
ggsave(glue("{figure_folder}/fig3a.pdf"),plot=sensor_neighbor_degree_plot,width=2.1,height=)
ggsave(glue("{figure_folder}/fig3b.pdf"),plot=sensor_connectivity_plot+theme(legend.position='none'),width=2.1,height=2.1)
ggsave(glue("{figure_folder}/fig3d.pdf"),plot=sens_deg_comp+theme(legend.position='none'),width=3.2,height=2.1)
ggsave(glue("{figure_folder}/fig3e.pdf"),plot=sens_sens_comp+theme(legend.position='none'),width=3.9,height=2.1)


ggsave(glue("{figure_folder}/fig4a.pdf"),plot=eff_neighbor_degree_plot,width=3,height=3)
ggsave(glue("{figure_folder}/fig4c.pdf"),plot=eff_deg_comp+theme(legend.position='none'),width=3,height=3)
ggsave(glue("{figure_folder}/fig4d.pdf"),plot=eff_eff_comp+theme(legend.position='none'),width=3,height=3)
...
```{r}
ggsave(glue("{figure_folder}/fig3b_withlegend.pdf"),plot=sensor_connectivity_plot,width=2.1,height=2.1)
...
```{r}
ggsave(glue("{figure_folder}/fig3e.pdf"),plot=sens_sens_comp+theme(legend.position='none'),width=3.2,height=2.1)
