...
print(cor.test(pcc_df$deg[pcc_df$deg<10],pcc_df$sens[pcc_df$deg<10],method='spearman',exact=FALSE))
```
...
pcc_eff_dens <-
  cowplot::ggdraw(plot_density_prs(pcc_df, eff_norm, xlab = "Effectiveness (a.u.)") + theme(legend.position = "none") +
...
pcc_sensitivity_dens <-
  cowplot::ggdraw(plot_density_prs(pcc_df, sens_norm, xlab = "Sensitivity (a.u.)") + theme(legend.position = "none")) #+
#        cowplot::draw_plot(dens_lgn, .55,.55,.5,.5)
...
ggsave(glue("{figure_folder}/fig2.png"),plot=combined_plot,width=10,height=12)
}
```
