...
```
# Plot heatmap
Using the clustering with `euclidian` distance and `ward` linkage, plot the heatmap with row and column colors are the GO term annotations found from effector and sensor analysis
...
clrs = map(py$eff_sens_go_list, ~ifelse(is.null(.x), 'none',.x))%>%unlist()

rowColors = rowAnnotation(rowColors=clrs,
                          col=list(rowColors = go_color_map ),
                          #width=unit(100,'mm'),
                          simple_anno_size=unit(3,'cm'),
                          show_annotation_name=FALSE,
                          show_legend=FALSE)
colColors = columnAnnotation(colColors=clrs,
...
# Import goatools functions from src/utils.py
...
effector_pcc <- unlist(eff_go_terms)%>%as.data.frame()%>%rownames_to_column()%>%mutate(rowname=as.numeric(rowname))%>%rename(cluster_go_term='.')%>%right_join(effector_pcc,by=c('rowname'='clusters'))
for(i in 1:4){
    print(i)
...
sensors_pcc <- unlist(sens_go_terms)%>%as.data.frame()%>%rownames_to_column()%>%mutate(rowname=as.numeric(rowname))%>%rename(cluster_go_term='.')%>%right_join(sensors_pcc,by=c('rowname'='clusters'))
