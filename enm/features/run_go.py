...
for i in tqdm(range(sim_num)):
    ...
    query = combined_df.iloc[np.argsort(np.abs(eig_vec)),:].loc[:,'Systematic gene name'].values[:260]#combined_df.sample(100).loc[:,'Systematic gene name'].values#
    ...
    goeaobj.wr_tsv(f'../data/interim/go_results_excl/{i}.tsv', goea_results_sig , itemid2name = geneid2name)
