...
# %%
snakemake = smake({'pickle_file_name':f'../data/interim/{nw_type}/{nw_type}_enm_object_corr.pickle',
                   'pcc_all':f'../data/interim/{nw_type}/{nw_type}_edgelist_corr.csv',
                   'gaf':"../data/raw/ontology/pombase.gaf", 'obo':'../data/raw/ontology/go-basic.obo', 'background_file':f'../data/interim/{nw_type}/{nw_type}_go_background_list_corr'})
...
goea, geneid2name,a = create_goea(gaf=snakemake.input['gaf'], 
                                  ...
                                  background=snakemake.input['background_file'], sgd_info_tab=None, project_folder='./',methods=['fdr'],
                                 ev_exclude={'ND','IGI','HGI'},
                                 goset=['BP'])

# %%
e_rew.get_sensor_effector(quantile_threshold=0.95)
...
for i in tqdm(e_pcc.e_list):
    ...
    i.get_sensor_effector(quantile_threshold=0.95)
...
# %%
sensors_df = pd.read_csv(f'../data/interim/{nw_type}/{nw_type}_sensors_df_corr.csv')
