...
rule clean_pombe_corr_data:
    input: 
        f"{RAW_INPUT_PATH}/{{nw_type}}/pombe_corr_raw",
    params: 
    ...
    conda:
        "enm_snakemake.yml"
    output: 
        f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_edgelist_corr.csv" ,
        f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_go_background_list_corr"
    shell:
...
rule rewire_network_pombe:
    input: f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_enm_object_corr.pickle"
    params:
        n_sim = N_SIM,
    conda:
        "enm_snakemake.yml"
    output: 
        pcc_df_random = f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_df_random_{N_SIM}_corr.csv"
    shell: "python3 scripts/rewiring.py --pickle_file {input[0]} --random_output_file {output.pcc_df_random} --n_sim {params.n_sim}"
...
rule effector_sensor_go_pombe:
    input:
        pickle_file_name= f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_enm_object_corr.pickle",
        gaf= f"{RAW_INPUT_PATH}/ontology/pombase.gaf",
        obo= f"{RAW_INPUT_PATH}/ontology/go-basic.obo",
        background_file = f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_go_background_list_corr"
        #sgd_info = None#f"{RAW_INPUT_PATH}/ontology/SGD_features.tab"
    conda:
        "enm_snakemake.yml"
    output:
        sensors_df_fname = f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_sensors_df_corr.csv",
        effectors_df_fname = f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_effectors_df_corr.csv",
        effector_sensor_combined_go_df = f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_effector_sensor_combined_go_df_corr.csv"
    shell:
...
rule figs2_pombe:
    input: 
        ...
        obo= f"{RAW_INPUT_PATH}/ontology/go-basic.obo",
        background_file = f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_go_background_list",
        pcc_all =f"{OUTPUT_PATH}/{{nw_type}}/{{nw_type}}_edgelist.csv" 
    output: 
        rewired_data_folder = directory('data/interim/{nw_type}/{nw_type}_rewired_data10test'),
        notebook="reports/05-Figs2_{nw_type}.ipynb"
    conda:
        "enm_snakemake.yml"
    log:
        notebook="reports/05-Figs2_{nw_type}.ipynb"
